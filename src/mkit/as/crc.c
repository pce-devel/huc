...
unsigned char ascii_lower_case[256] =
{
...
#else
	0x78u, 0x79u, 0x7Au, 0x5Bu, 0x5Cu, 0x5Du, 0x5Eu, 0x5Fu, // "XYZ[\]^_"
#endif
 ...
 * crc_calc
 * ----
 * the crc32 calculation as used in ethernet, pkzip, etc.
 */

unsigned int
crc_calc(const unsigned char *data, int len)
{
 ...
 * characters 0x80-0xFF = unchanged
 *
 * this also remaps the DOS path separator "\" to the UNIX "/".
 */

unsigned int
filename_crc(const char *name)
{
...
#if defined(_WIN32) || defined(__APPLE__)
	while ((byte = *data++) != 0)
		crc = crc32_table[((crc ^ ((unsigned int)ascii_lower_case[byte])) & 0xFFu)] ^ (crc >> 8);
#else
	while ((byte = *data++) != 0)
		crc = crc32_table[((crc ^ ((unsigned int)byte)) & 0xFFu)] ^ (crc >> 8);
#endif
