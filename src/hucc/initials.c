...
void add_symbol_initials (char *symbol_name, char type)
{
	...
	initials_table[initials_idx].init_type = type;
}
...
int find_symbol_initials (char *symbol_name)
{
	...
	initials_data_idx = 0;
	for (initials_idx = 0; initials_table[initials_idx].init_type != 0; initials_idx++) {
...
void add_data_initials (char *symbol_name, int type, int value, TAG_SYMBOL *tag)
{
	...
	if (tag != 0) {
		...
		int member_type = member_table[tag->member_idx + index].sym_type;
		// add it recursively
	...
	if (find_symbol_initials(symbol_name) != 0) {
		if ((initials_table[initials_idx].init_type & ~CUNSIGNED) == CCHAR)
			...
			for (i = 0; i < (position % number_of_members); i++) {
				type = member_table[tag->member_idx + i].sym_type;
				if (type == CCHAR || type == CUCHAR)
					index += 1;
				else
					index += INTSIZE;
