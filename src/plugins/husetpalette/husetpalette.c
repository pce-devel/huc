...
// husetpalette.c
//
// A simple Windows DDE plugin for Cosmigo's "Pro Motion NG" using a Dialog
// Box for user input.
//
...
// **************************************************************************
//
// Set all pixels to a user-chosen sub-palette number (0..15).
//
// **************************************************************************
// **************************************************************************
...
const char * const pPluginName = "Set Pixels to Palette";
...
//
// Set all non-transparent pixels to a specific sub-palette (0..15).
//
...
BOOL DoPluginAction( void )
{
    ...
    //
    // N.B. This has to be repeated because the project could be closed
    // while the dialog box is waiting for user input.
...
// **************************************************************************
//
// Check that "Pro Motion NG" is ready before activating the Dialog Box UI.

BOOL IsPluginReady( void )
{
    int         iLayerNum;
    char        aReply[256];
...
INT_PTR CALLBACK DialogProc( HWND hWindow, UINT uMessage, WPARAM wParam, LPARAM lParam )
{
    switch (uMessage)
    {
    case WM_COMMAND:
        ...
        if (LOWORD( wParam ) >= ID_KEYPAD_00 &&
            LOWORD( wParam ) <= ID_KEYPAD_0F) {
            uWhichPalette = 0x0F & (LOWORD( wParam ) - ID_KEYPAD_00);
    ...
    HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR pCmdLine, int nCmdShow)
{
    ...
    // Create a modeless dialog box for the plugin's UI.

    hMainWindow = CreateDialogParam( hInstance, MAKEINTRESOURCE( IDD_WHICH ), 0, DialogProc, 0 );

    // Open the connection to the Pro Motion DDE server.

    if (DdeOpen()) {
        PostMessage( hMainWindow, APP_TEST_READY, nCmdShow, 0 );
    } else {
    ...
    {
        if (!IsDialogMessage( hMainWindow, &cMessage ))
        {
            TranslateMessage( &cMessage );
            DispatchMessage( &cMessage );
        }
