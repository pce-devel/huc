...
// **************************************************************************
//
// Remap pixel values (and the palette) using a set of 256 numbers to define
// which pixel in the original image becomes which pixel value in the result.
//
// The equivalent in C is "new_pixel = remap_table[ old_pixel ];"
//
// The remapping table is read from the text file "remap.txt" that is in the
// directory as the current image in Pro Motion.
//
// A sample "remap.txt" file that just sets every pixel to its current value
// is provided as an example, and as a template to be copied and modified.
//
// **************************************************************************
// **************************************************************************
...
const char * const pPluginName = "Remap Pixel Values";
...
BOOL ProcessFrame( BITMAPINFO *pDIB8 )
{
    ...
    memcpy( aPalette, pDIB8->bmiColors, sizeof(aPalette) );
    for (uX = 0; uX != 256; ++uX) {
        aPalette[aRemap[uX]] = pDIB8->bmiColors[uX];
    }
    memcpy( pDIB8->bmiColors, aPalette, sizeof(aPalette) );
...
BOOL DoPluginAction( void )
{
    ...
    // Find out the current filename (including path).

    if (!DdeSendString( "GetCurrentFilename" ))
        goto error;
    if ((!DdeRecvString( aReply, sizeof(aReply) )) || (aReply[0] == '-'))
        goto error;

    if (strcmp( aReply, "none" ) == 0) {
        MessageBox( hMainWindow, "Current image has no filename!",
            pPluginName, MB_ICONEXCLAMATION | MB_OK );
        bShowDefaultError = FALSE;
        goto error;
    }

    // Read in the remapping table.

    if ((pReply = strrchr( aReply, '\\' )) != NULL) {
        pReply[1] = '\0';
    }
    if (strlen( aReply ) + 7 >= sizeof(aReply)) {
        MessageBox( hMainWindow, "Filename too long!",
           pPluginName, MB_ICONEXCLAMATION | MB_OK );
        bShowDefaultError = FALSE;
        goto error;
    } else {
        int iValue;
        unsigned uIndex = 0;
        strcat( aReply, "remap.txt" );
        if ((pFile = fopen( aReply, "r" )) == NULL) {
            MessageBox( hMainWindow, "Cannot open \"remap.txt\" file!",
               pPluginName, MB_ICONEXCLAMATION | MB_OK );
            bShowDefaultError = FALSE;
            goto error;
        }
        while (fscanf( pFile, " %i", &iValue ) == 1) {
            aRemap[uIndex] = iValue;
            if (++uIndex == 256) break;
        }
        fclose( pFile );
        if (uIndex != 256) {
            MessageBox( hMainWindow, "The \"remap.txt\" file contains less than 256 values!",
               pPluginName, MB_ICONEXCLAMATION | MB_OK );
            bShowDefaultError = FALSE;
            goto error;
        }
