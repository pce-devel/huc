...
Extracts one or more **character** patterns (tiles of 8x8 pixels) from a picture file. Extracts '*col*' columns and '*row*' rows of tiles (in character units), starting at position '*begin_x*' and '*begin_y*' (in pixels). Maximum number of characters is 2048. The optional `_OPTIMIZE` parameter is used by HuCC for the newer Character Map Functions to automatically optimize your character set (i.e. remove duplicates).
...
Extracts one or more **sub-palettes** (groups of 16 colors) from a picture file. Up to 32 sub-palettes can be imported: 16 for tiles (0-15), 16 for sprites (16-31).
...
Sets the base bank and address for far memory operations. Used in conjunction with far memory functions like `far_load_vram()`, `far_load_bat()`, etc.
