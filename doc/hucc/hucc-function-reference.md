...
HuCC supports the following 8-bit picture formats: BMP, PCX, PNG.

`#incbin( identifier_name, "filename" );`
...
Extracts one or more **character** patterns (tiles of 8x8 pixels) from a picture file. Extracts '*col*' columns and '*row*' rows of tiles (in characters), starting at position '*begin_x*' and '*begin_y*' (in pixels). Maximum number of characters is 2048.

**Note 1:** The optional `_OPTIMIZE` parameter is used by HuCC for the newer Character Map Functions to automatically optimize your character set (i.e. remove duplicates).

**Note 2:** The extraction coordinates '*begin_x*' and '*begin_y*' are only useful for the older Tile and Map Functions from HuC3/4. When the newer `_OPTIMIZE` parameter is specified, those legacy coordinates **must** be set to `0, 0`!
...
**Note 1:** The '*blur_flag*' parameter only works with a composite output. `XRES_SHARP` provides a clearer image, but it can generate visible artifacts with horizontal resolutions higher than 256 pixels.
...
Creates a split screen window at the specified screen line. Up to 128 windows can be defined for VDC1. '*index*' is the split index (0-127), '*screen_line*' is the physical line where the split occurs (0-239), '*bat_x*' and '*bat_y*' are the top-left coordinates of the area in the virtual screen that will be displayed in the window, and the '*display_flags*' control what is displayed (`BKG_ON`, `SPR_ON`, `BKG_OFF`, `SPR_OFF`).
...
Sets the sub-palette index (0-15) of the current sprite.
...
Draws a specific rectangular area from the character map to the BAT. Source coordinates are provided by the HuCC variables '*vdc_map_pxl_x*' and '*vdc_map_pxl_y*' (in pixels). Parameters specify the destination coordinates and dimensions (in characters).
...
Block map functions provide a map system based on 16x16 blocks (metatiles). This system is more efficient for large maps and supports collision detection.
...
- '*tile_type'* : Type of tiles (`8` or `16`).
...
Returns the tile index as defined in the tile array used in the most recent call to `set_tile_data()`. The '*x*' and '*y*' arguments are in pixels, unlike the put_tile functions. So this function is ideal for collision routines.
...
All these legacy text output functions have two forms: one where you directly specify the VRAM address, and another one where you specify '*x*' and '*y*' coordinates (in characters). The second form is a bit slower but more user-friendly.
...
`cd_loaddata( unsigned char ovl_index, unsigned int sect_offset, unsigned char __far *destaddr, unsigned int bytes );`
Reads data from the CD-ROM into a destination area (or overlay 'const' or other data) specified by '*destaddr*', for a length of '*bytes*'. Reads it from the overlay segment specified by '*ovl_index*', with sector offset (i.e. multiples of 2048 bytes) of '*sect_offset*'. Non-zero return values indicate errors.
